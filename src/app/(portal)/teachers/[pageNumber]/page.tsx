...
import { PageProps } from '../../school-classes/[pageNumber]/page';
...
const TeachersPage = async ({ params }: PageProps) => {
  ...
  try {
    ...
    if (res && !isErrorResponse(res) && res.data) {
      ...
      totalPages =
        typeof res.data.total_pages === 'number' ? res.data.total_pages : 0;
      totalTeachers =
      ...
      console.warn(
        'Error response from teachers API:',
        res.status,
        res.message,
      );
    }
  ...
  try {
    const analyticsRes = await getModel<TeachersAnalyticsResponse>(
      '/analytics/tenant/teachers',
    );
    if (analyticsRes && !isErrorResponse(analyticsRes) && analyticsRes.data) {
      ...
      if (analyticsRes.status !== 404) {
        console.warn(
          'Error response from teachers analytics API:',
          analyticsRes.status,
  ...
  const transformedData: Teacher[] =
    teachers?.map((teacher: BackendTeachersData) => {
