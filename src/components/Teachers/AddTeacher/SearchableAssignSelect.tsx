...
export default function SearchableAssignSelect({
    ...
    const [open, setOpen] = useState(false) // <-- NEW: controls visibility
    ...
    const toggleOption = (option: MultiSelectOption) => {
        ...
        setQuery('')
        setOpen(false) // <-- Close after selecting (optional)
    }
    ...
    return (
        <div ref={containerRef} onBlur={handleBlur} className="space-y-4 relative">
            ...
            <div className="relative">
                ...
                {open && (
                    <div
                        className="absolute top-10 z-10 mt-1 w-full max-h-64 overflow-y-auto rounded-md border border-zinc-200 bg-white shadow-sm"
                    >
                        {loading ? (
                                    ...
                                    return (
                                        <li key={option.id}>
                                            <button
                                                ...
                                                onMouseEnter={() => setHighlightedIndex(index)}  // <-- NEW: update highlight on hover
                                                className={clsx(
                        ...
                        <ul className="mt-3 gap-2 border border-zinc-200 rounded-md divide-y divide-zinc-200 ">
                            {selected.map((option) => (
                                <li key={option.id} className="flex items-center justify-between py-1 px-3 ">
                                    ...
                                    <button type="button" aria-label={`Remove ${option.name}`} onClick={() => removeSelection(option.id)} className="rounded-md p-2 text-zinc-800 hover:bg-zinc-100">
